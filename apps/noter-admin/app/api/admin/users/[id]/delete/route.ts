 ...
 *
 * 软删除用户:SET profiles.deleted = 1(不级联删除文档等关联数据）
 *
 ...
 *       操作自身 → 409
 *   - 成功后写 audit log (action_type: 'user.delete', target_resource_type: 'user')
 */
...
async function handler(request: Request, ctx: unknown): Promise<Response> {
  ...
  if (updateError) {
    throw new Error(`软删除用户失败: ${updateError.message}`)
  }
